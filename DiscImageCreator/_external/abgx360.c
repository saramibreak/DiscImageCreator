...
void printwin32filetime(unsigned long long win32filetime, char* date) {
	...
	if (days == -1) {
		sprintf(date, "%d-12-31T%02llu:%02llu:%02llu", year - 1, hours % 24, minutes % 60, seconds % 60);
	}
	else {
		...
		sprintf(date, "%d-%02i-%02luT%02llu:%02llu:%02llu", year, month, days, hours % 24, minutes % 60, seconds % 60);
	}
	return;
}
