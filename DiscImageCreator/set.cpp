...
VOID SetAndOutputTocCDText(
	...
	if (uiPacksOfAlbum) {
		...
		for (UINT i = 0; i < uiLastTrackNum + 1; i++) {
			...
			if (i == 0) {
				OutputDiscLogA("\tAlbum Name: %s\n", pDisc->SCSI.CDTEXT[nIdx].pszTitle[i]);
			}
			else {
				OutputDiscLogA("\t Song Name[%d]: %s\n", i, pDisc->SCSI.CDTEXT[nIdx].pszTitle[i]);
			}
			if (bUnicode) {
				uiTxtIdx += 2;
			}
	...
	if (uiPacksOfPerformer) {
		...
		for (UINT i = 0; i < uiLastTrackNum + 1; i++) {
			...
			if (i == 0) {
				OutputDiscLogA("\tAlbum Performer: %s\n", pDisc->SCSI.CDTEXT[nIdx].pszPerformer[i]);
			}
			else {
				OutputDiscLogA("\t Song Performer[%d]: %s\n", i, pDisc->SCSI.CDTEXT[nIdx].pszPerformer[i]);
			}
			if (bUnicode) {
				uiTxtIdx += 2;
			}
	...
	if (uiPacksOfSongwriter) {
		...
		for (UINT i = 0; i < uiLastTrackNum + 1; i++) {
			...
			if (i == 0) {
				OutputDiscLogA("\tAlbum SongWriter: %s\n", pDisc->SCSI.CDTEXT[nIdx].pszSongWriter[i]);
			}
			else {
				OutputDiscLogA("\t      SongWriter[%d]: %s\n", i, pDisc->SCSI.CDTEXT[nIdx].pszSongWriter[i]);
			}
			if (bUnicode) {
				uiTxtIdx += 2;
			}
		...
		if ((pDiscPerSector->subch.prev.byCtl & AUDIO_DATA_TRACK) == 0) {
			...
			if ((pDiscPerSector->subch.current.byCtl & AUDIO_DATA_TRACK) == 0) {
				if (pDiscPerSector->subch.prev.byTrackNum > 0) {
					if (nLBA == pDisc->SCSI.lpFirstLBAListOnToc[pDiscPerSector->subch.prev.byTrackNum] - 225) {
					...
					else if (nLBA == pDisc->SCSI.lpFirstLBAListOnToc[pDiscPerSector->subch.prev.byTrackNum] - 150) {
						pDiscPerSector->subch.current.nRelativeTime = 149;
						bValidPre = TRUE;
					}
					else if (nLBA == pDisc->SCSI.lpFirstLBAListOnToc[pDiscPerSector->subch.prev.byTrackNum] - 149) {
